...
export const normalizeEmployeeData = (employee: EmployeeData): EmployeeData => ({
  ...
  buddyShiftsRemaining: Number.isFinite(employee.buddyShiftsRemaining)
    ? Math.max(0, Math.round(employee.buddyShiftsRemaining))
    : 0,
  contractHours:
