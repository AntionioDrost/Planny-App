...
export interface EmployeeScheduleStats {
  ...
  totalBuddyHours?: number;
  buddyShiftCount?: number;
}
...
export interface ScheduleResult {
  ...
  buddyProgress?: Record<string, { requested: number; assigned: number; remaining: number; completionDay: string | null; regularStartDay: string | null }>;
}
...
export function generateSchedule(
  ...
  for (let i = 0; i < runCount; i++) {
    const currentSchedule = generateBuddySchedule(data);
  ...
  // ponytail: skip the final rebalance when buddy rules are active; that pass treats buddy shifts as regular hours.
  return data.employees.some(employee => (employee.buddyShiftsRemaining ?? 0) > 0)
    ? bestSchedule!
    : rebalanceShiftsForAbsoluteHours(data, bestSchedule!);
}

const generateBuddySchedule = (data: ParsedData): ScheduleResult => {
  const requested = Object.fromEntries(data.employees.map(e => [e.name, Math.max(0, Math.round(e.buddyShiftsRemaining || 0))]));
  let starts = Object.fromEntries(data.employees.map(e => [e.name, requested[e.name] ? data.days.length : 0]));
  let result: ScheduleResult = generateSingleSchedule(data);
  let progress = buildBuddyProgress(data, result.employeeSchedules, requested);

  for (let pass = 0; pass < Math.max(3, data.days.length + 2); pass++) {
    const gated = {
      ...data,
      employees: data.employees.map(employee => ({
        ...employee,
        availability: Object.fromEntries(data.days.map((day, index) => [day, index >= starts[employee.name] ? employee.availability[day] : '']))
      }))
    };
    result = generateSingleSchedule(gated);
    progress = buildBuddyProgress(data, result.employeeSchedules, requested);
    const nextStarts = Object.fromEntries(data.employees.map(employee => {
      const p = progress[employee.name];
      return [employee.name, p.remaining ? data.days.length : data.days.findIndex(day => day === p.regularStartDay) >= 0 ? data.days.findIndex(day => day === p.regularStartDay) : 0];
    }));
    if (JSON.stringify(nextStarts) === JSON.stringify(starts)) break;
    starts = nextStarts;
  }
  const rebuilt = buildScheduleResultFromAssignments(data, result.employeeSchedules, {
    normal: result.unfilledNormalShifts,
    web: result.unfilledWebShifts,
    revision: result.unfilledWebRevisionShifts,
  });
  rebuilt.buddyProgress = progress;
  return rebuilt;
};

const buildBuddyProgress = (data: ParsedData, schedules: Record<string, AssignedShift[]>, requested: Record<string, number>) => {
  const progress: NonNullable<ScheduleResult['buddyProgress']> = {};
  for (const employee of data.employees) {
    const wanted = requested[employee.name];
    const existing = (schedules[employee.name] ?? []).filter(shift => !shift.isWeb && !shift.isWebRevision && !shift.isBuddy);
    const assigned = Math.min(wanted, existing.length);
    const completionIndex = assigned === wanted && wanted > 0 ? data.days.findIndex(day => day === existing[assigned - 1]?.day) : -1;
    const startIndex = completionIndex >= 0 ? completionIndex + 1 : wanted ? data.days.length : 0;
    const buddyShifts: AssignedShift[] = [];
    const buddyDays = new Set<string>();
    if (wanted) {
      for (const day of data.days) for (const type of ['Morning', 'Afternoon'] as ShiftType[]) {
        if (buddyShifts.length >= wanted || data.closedDays.includes(day) || !isAvailable(employee, day, type, data.closedDays)) continue;
        if (buddyDays.has(day)) continue;
        const slot = Object.values(schedules).flat().filter(s => s.day === day && s.type === type && !s.isBuddy && !s.isWeb && !s.isWebRevision);
        if (slot.length !== NORMAL_SHIFT_CAPACITY || (schedules[employee.name] ?? []).some(s => s.day === day && s.type === type)) continue;
        buddyShifts.push({ day, type, isWeb: false, isWebRevision: false, isBuddy: true });
        buddyDays.add(day);
      }
      schedules[employee.name] = [...(schedules[employee.name] ?? []), ...buddyShifts];
    }
    const finalAssigned = buddyShifts.length;
    const finalCompletion = finalAssigned === wanted && wanted > 0 ? buddyShifts[finalAssigned - 1].day : null;
    progress[employee.name] = { requested: wanted, assigned: finalAssigned, remaining: Math.max(0, wanted - finalAssigned), completionDay: finalCompletion, regularStartDay: finalCompletion ? data.days[data.days.indexOf(finalCompletion) + 1] ?? null : null };
  }
  return progress;
};
...
export function buildScheduleResultFromAssignments(
  ...
  for (const employee of data.employees) {
    ...
    for (const shift of normalizedShifts) {
      ...
      if (shift.isBuddy) {
        stats[employee.name].totalBuddyHours = (stats[employee.name].totalBuddyHours ?? 0) + HOURS_PER_SHIFT;
        stats[employee.name].buddyShiftCount = (stats[employee.name].buddyShiftCount ?? 0) + 1;
      } else {
