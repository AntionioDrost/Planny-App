                            ...
                            {section.slots.length === 0 ? (
                              ...
                              <ul className="space-y-2">
                                {section.slots.map(slot => {
                                  ...
                                  const isBuddySlot = slot.employeeName
                                    ? schedule.employeeSchedules[slot.employeeName]?.some(
                                        shift => shift.day === slot.day && shift.type === slot.type && shift.isBuddy
                                      )
                                    : false;
                                  const meta = isBuddySlot
                                    ? { ...SLOT_KIND_META.normal, label: 'BUDDY', tone: 'bg-amber-100 text-amber-800 border border-amber-200' }
                                    : SLOT_KIND_META[slot.kind];
