...
export function ConfigPanel({
  ...
  const [activeTab, setActiveTab] = useState<'main' | 'web' | 'fullDays' | 'buddy'>('main');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex border-b border-slate-200">
        ...
        <button
          className={`px-6 py-3 font-medium text-sm transition-colors border-b-2 ${activeTab === 'buddy' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}
      ...
      {activeTab === 'buddy' && (
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="mb-5 text-sm text-slate-600">A buddy loopt als extra persoon mee op een volledig bezette reguliere shift.</p>
          <div className="overflow-x-auto"><table className="w-full text-sm text-left">
            <thead className="border-b border-slate-200 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Name</th><th className="px-4 py-3">Remaining buddyshifts</th><th className="px-4 py-3">Status</th></tr></thead>
            <tbody className="divide-y divide-slate-100">{data.employees.map((emp, idx) => <tr key={idx}>
              <td className="px-4 py-3 font-medium">{emp.name}</td>
              <td className="px-4 py-3"><input min="0" step="1" type="number" value={emp.buddyShiftsRemaining ?? 0} onChange={event => updateEmployee(idx, employee => ({ ...employee, buddyShiftsRemaining: Math.max(0, Math.round(Number(event.target.value) || 0)) }))} className="w-28 rounded-md border border-slate-300 px-2 py-1.5" /></td>
              <td className="px-4 py-3">{(emp.buddyShiftsRemaining ?? 0) > 0 ? 'Training' : 'Regular'}</td>
            </tr>)}</tbody>
          </table></div>
        </div>
      )}
